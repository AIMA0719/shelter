...
| 항목 | 현재 | 운영 필요 |
|---|---|---|
| **건물 높이** | 샘플 + `fetch_district.py`(OSM) + **`shp_to_buildings.py`(GIS건물통합정보)** | GIS건물통합정보 SHP(EPSG:5186, A16=실측높이/A26=층수) → `shp_to_buildings.py` 로 GeoJSON 변환(권역 `--bbox` 또는 전체). 전체는 PostGIS 적재 |
